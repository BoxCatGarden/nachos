...
AddrSpace::~AddrSpace()
{
    cout << "release address space" << endl;
    if (pageTable)
        kernel->memory->VirRelease(pageTable, (int)numPages);
}

void AddrSpace::operator delete(void *p)
{
    if (--((AddrSpace *)p)->counter < 1)
        ::delete p;
}
...
bool AddrSpace::Load(char *fileName)
{
    ...
    if (noffH.initData.size > 0)
    {
        ...
        kernel->memory->VirLoad(pageTable, noffH.initData.virtualAddr,
                executable, noffH.initData.size, noffH.initData.inFileAddr);
    }
    ...
    if (noffH.readonlyData.size > 0)
    {
        ...
        kernel->memory->VirLoad(pageTable, noffH.readonlyData.virtualAddr,
                executable, noffH.readonlyData.size, noffH.readonlyData.inFileAddr);
    }
#endif
...
{
    kernel->machine->processPageTable = pageTable;
