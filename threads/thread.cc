...
void Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    DEBUG(dbgThread, "Forking thread: " << name << " id: " << id << " f(a): " << (int)func << " " << arg);

    ASSERT(id != InvalidThreadId); //a thread with invalid id shouldn't be forked,
                                //which, instead, should be tested at the time it created
                                //and just deleted then, as in the kernel
...
void Thread::Finish()
{
    ...
    OnFinish(); //finish all work before sleeping forever
...
/*----------------------------------------------------------------------
 * do all the jods that must be done in the current thread
----------------------------------------------------------------------*/
void Thread::OnFinish()
{
    kernel->globalMailbox->Clear(); //clear all the global mails sent to the current thread
}
...
//static
//assume that the interrupt is disabled.
 ...
 * 
 * Note: assume that the interrupt is disabled.
------------------------------------------------------*/
